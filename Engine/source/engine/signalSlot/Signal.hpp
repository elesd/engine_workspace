...
namespace engine
{
	namespace signalSlot
	{
		...
		namespace _private
		{
			...
			template<class T, typename ...Args>
			template<void (T::*func)(Args...)>
			uint32_t MemberFunctionId<T, Args...>::Id<func>::value = engine::utils::IdGenerator<MEMBER_FUNCTION>::generateNextId();
		}
		...
		void Signal<Args...>::connect(T *holder)
		{
			...
			using IdClass = typename engine::signalSlot::_private::template MemberFunctionId<T, Args...>::template Id<slot>;
