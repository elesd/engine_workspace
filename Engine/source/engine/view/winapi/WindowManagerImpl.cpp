	...
	namespace view
	{
		...
		namespace winapi
		{
			...
			view::Window *WindowManagerImpl::createMainWindowImpl(const view::WindowParameter &parameters, const std::string &title)
			{
				...
				const app::winapi::WinApiApplicationParameter *args = static_cast<const app::winapi::WinApiApplicationParameter*>(arguments);
			...
			std::unique_ptr<video::Driver> WindowManagerImpl::createDriverForWindow(const video::DriverInitParameters &params, Window *window) const
			{
				std::unique_ptr<video::Driver> driver(new video::winapi::DriverImpl());
